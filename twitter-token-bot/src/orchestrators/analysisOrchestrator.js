// Updated AnalysisOrchestrator - WEBHOOK FUNCTIONALITY REMOVED
...
class AnalysisOrchestrator {
    constructor(config = {}) {
        ...
        this.config = {
            ...
            saveToJson: config.saveToJson !== false,
            ...config
        };
        ...
        logger.info(`📱 Using Telegram-only publishing`);
    ...
    // Simple Telegram-only publishing
    async publishResults(analysisResult) {
        try {
            const { operationId } = analysisResult;
            logger.info(`📤 [${operationId}] Publishing analysis to Telegram`);
            await this.telegramPublisher.publishAnalysis(analysisResult);
            logger.info(`✅ [${operationId}] Published to Telegram successfully`);
        } catch (error) {
            logger.error(`❌ [${analysisResult.operationId}] Failed to publish to Telegram:`, error);
            throw error;
        }
    }
